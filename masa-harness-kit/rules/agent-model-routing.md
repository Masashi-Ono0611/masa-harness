...
| Agent (subagent_type) | デフォルト model | 上位 model に上げる条件 |
|---|---|---|
...
★ **現行**: 旧ルールは Opus デフォルト。月次リミット超過の実績を受け Sonnet に変更。
...
|---|---|
| Plan → Sonnet / Opus | Plan → 骨格確認のみ Haiku／実装計画は Sonnet |
...
> ユーザーが「節約モード解除」「通常モードで」と言えば解除。自動で固定しない。
