...
|---|---|---|---|
| settings.json / hooks / 構造 | `claude-config-audit` | 公式ベストプラクティスとの構造ギャップ、hook 健全性 | skill 中身・memory・skills sprawl |
...
> **`claude-stack-audit` は本マップの対象外**（混同注意）: これは hygiene（環境保守）skill では**なく**、「**新着アップデート起点**」で新機能を「導入すべき/様子見/不要」に分類する採用判断＋セキュリティ監査 skill。「**公式ベストプラクティス起点**」で設定の健全性を診る `claude-config-audit` とは**起点も目的も別物**。名前が `*-audit` で似ているだけで owner レーンを持たない。

## best practice（2026 調査・実行基準）

- **グローバル規則は CLAUDE.md（lean・常時）／ドメイン手順は skill**（Thin Harness, Fat Skills と一致）
- **skill sprawl は 8–12 個が注意域・layer ごと >10 で audit 発火**。超過分は "context tax"（使わなくても払うトークン）。**未使用 skill は即 disable**
...
## cadence（週次をダウングレードしない・効く範囲で軽く最適化のみ）
...
- まとめ点検時の個別実行（§owner マップ参照）は週次 due の**置換でなく上乗せ**。明確に効く範囲でのみ軽く最適化する（過剰な統合をしない）。なお `claude-stack-audit` は hygiene でなく対象外（§owner マップに従う）
