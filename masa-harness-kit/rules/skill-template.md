...
description: |-
  ...
  <スコープ / 適用範囲>。
scope: local-only       # 任意。個人ローカル skill (gitignore 配下) なら明示
...
| **小数ステップ採番** (`Step 2.5` 等) | 整数振り直し or `Na/Nb` (正本: CLAUDE.md §共通ルール 採番 reflex) | 暫定感が残り増殖・番号を ID 扱いするのに不安定 |
...
- 本文の Constraints / Best Practices から `[E1]` で双方向リンク
